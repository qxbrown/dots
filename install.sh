...
install_paru() {
    ...
    # Changed: paru-bin → paru (source-built, more stable with system updates)
...
# Added: rebuild paru if it's broken before installing AUR packages
if ! paru --version &>/dev/null; then
    echo -e "${BLUE}paru is broken, rebuilding it from AUR...${RESET}"
    TMPDIR=$(mktemp -d)
    git clone https://aur.archlinux.org/paru.git "$TMPDIR/paru"
    cd "$TMPDIR/paru"
    makepkg -si --noconfirm
